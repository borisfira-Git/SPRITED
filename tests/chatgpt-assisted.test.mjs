...
test('ChatGPT Assisted prepares, imports, validates and repairs through shared storage',async()=>{
  ...
  try{
    ...
    const gait=await service.call('agent/prepare-assisted-request',{animation_id:animationId,frame_index:0,request_mode:'full_gait',quality_mode:'quality'});assert.equal(gait.result.quality_mode,'quality');assert.match(gait.result.prompt,/1\. left contact/);assert.match(gait.result.prompt,/8\. right up/);assert.match(gait.result.prompt,/preserve anatomical left\/right/i);
    const fast=await service.call('agent/prepare-assisted-request',{animation_id:animationId,frame_index:1,request_mode:'next_frame',quality_mode:'fast'});assert.match(fast.result.prompt,/Advance the motion by one phase only/);assert.match(fast.result.prompt,/Fast mode/);
