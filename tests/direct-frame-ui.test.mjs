...
try{
  ...
  assert.deepEqual(errors,[]);assert.equal(await page.locator('#libraryVideoPreview').count(),0);assert.equal(await page.locator('video').count(),0);assert.equal(await page.locator('.creator-shell').count(),1);assert.equal(await page.locator('.step').count(),0);assert.equal(await page.locator('#animationCards button').count(),6);assert.equal(await page.locator('#libraryAdvanced').getAttribute('open'),null);assert.equal(await page.locator('#libraryProvider').isVisible(),false);assert.match(await page.locator('#resultCounter').textContent(),/Result 2 of 2/);
