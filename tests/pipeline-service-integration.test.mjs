...
test('SPRITED service stops a generated sequence until real motion observations exist',async()=>{
  ...
  try{
    ...
    const result=await service.call('pipeline/run',{character_id:character.id,animation_type:'WALKING',frames:8,cols:4,provider:'cloudflare_flux',repair_intelligence:'sprited_only',confirm_paid_request:true});assert.equal(result.success,true,JSON.stringify(result));assert.equal(result.status,'motion_observation_required');assert.equal(calls.filter(item=>item[0]==='generate').length,8);const run=await service.core('workflow/animation/status',{id:result.result.animation_id});assert.equal(run.frame_records.length,8);assert.equal(new Set(run.frame_records.map(frame=>frame.frame_id)).size,8);assert.equal(run.technical_validation.passed,true);assert.equal(run.technical_validation.loop_validation.passed,true);assert.equal(run.temporal_inspection.status,'INSUFFICIENT_MOTION_OBSERVATION');assert.equal((run.gif_previews||[]).length,0);assert.equal((run.spritesheets||[]).length,0);
    const approved=await service.call('agent/use-result',{id:run.id});assert.equal(approved.success,false);assert.match(approved.errors[0],/temporal validation/i);
