...
(() => {
  ...
  function create(w,type) {
    ...
    const run={id:id(),character_profile_id:c.id,character_reference:copy(c),animation_type:type,recipe_id:recipe.id,recipe_snapshot:copy(recipe),status:'draft',source_mode:'manual_video',source_video_path:null,
  ...
  function queue(w,r,previous=null){
    ...
    r.source_mode='agent_router';r.selected_provider='agent_router';r.status='queued';r.source_video_path=null;
    const j={job_id:id(),attempt_id:r.id,character_id:r.character_profile_id,character_name:r.character_reference.name,reference_image_path:r.character_reference.reference_image_path,reference_image:r.character_reference.reference_image,
      ...
      generation_constraints:['Preserve face, hairstyle, clothing, colors, weapons, accessories, proportions and silhouette','Fixed camera and view direction, full body visible, no cropping','Generate continuous temporal motion, never independent final sprites',...(r.options.loop?['Produce a complete seamless loop; do not mirror anatomical left/right']:['Include a readable final resting pose']),...(r.animation_type==='WALKING'?['Opposite leg must lead in the second half of the gait cycle']:[])],
      preferred_output_format:'video',output_directory:`SPRITED_DATA/characters/${r.character_profile_id}/animations/${r.animation_type.toLowerCase()}/${r.id}`,status:'QUEUED',claimed_by:null,claim_token:null,claimed_at:null,retry_count:0,created_at:now(),updated_at:now(),warnings:[],errors:[],variation_request:previous?{previous_attempt_id:previous.id,rejection_reason:previous.rejection_reason||null,instruction:'Generate a genuinely different result; use a new seed/variation when supported. Do not reuse the previous video.'}:null};
