...
(() => {
  ...
  function regenerate(w,runId){const old=get(w,runId);if(w.animation_runs.length>=50)throw Error('Maximum 50 runs');const r=copy(old);r.id=id();r.parent_run_id=old.id;r.created_at=now();invalidate(r);Object.assign(r,{status:'draft',approval_state:'pending',source_video_path:null,source_frame_paths:[],frame_records:[],selected_provider:null,provider_metadata:{},router_decision:null,technical_validation:null,semantic_validation:null,combined_validation:null,current_preview_id:null,current_contact_sheet_id:null,repair_plans:[],repair_history:[],semantic_reinspection_required:false});w.animation_runs.push(r);return copy(r);}
