...
(() => {
  ...
  const recipes = definitions.map(([animation_type,display_name,loop,source,output,alignment,rules]) => ({
    ...
    configurable_options:['source_frames','output_frames','canvas_width','canvas_height','alignment','background_mode','background','start','end','cols','quality_mode','direction']
  }));
  ...
  function settings(values) {
    ...
    if(s.quality_mode!==undefined&&!['fast','quality'].includes(s.quality_mode))throw Error('Unknown quality mode');
    if(s.direction!==undefined&&!['reference','left','right'].includes(s.direction))throw Error('Unknown direction');
    return s;
  }
  function image(src) {
  ...
  function hydrate(value) {
    ...
    for(const r of w.animation_runs){
      ...
      r.frame_records ||= [];r.provider_usage||={paid_generations:0,paid_repairs:0};r.retention||={keep_final_spritesheet:true,keep_final_frames:false,keep_failed_frames:false,keep_contact_sheets:false,keep_preview_gifs:false,keep_experience_history:true,pinned_artifact_ids:[]};r.storage_state||='temporary';r.cleanup_pending=Boolean(r.cleanup_pending);if(!['paid_generations','paid_repairs'].every(key=>Number.isInteger(r.provider_usage[key])&&r.provider_usage[key]>=0))throw Error('Invalid provider usage');if(!Array.isArray(r.frame_records)||r.frame_records.length>24)throw Error('Invalid stored frame records');
      ...
      for(const sheet of r.spritesheets||[])if(!sheet.artifacts&&Array.isArray(sheet.output_paths)){const roles=['final_spritesheet','spritesheet_metadata','editable_project','export_manifest'];sheet.artifacts=sheet.output_paths.map((output_path,index)=>({artifact_id:id(),owner_animation_id:r.id,sheet_id:sheet.id,role:roles[index]||'frame_copy',storage_path:output_path,output_path,pinned:false}));}
  ...
  function create(w,type) {
    ...
    const run={id:id(),character_profile_id:c.id,character_reference:copy(c),animation_type:type,recipe_id:recipe.id,recipe_snapshot:copy(recipe),status:'draft',source_mode:'direct_frames',source_video_path:null,source_frame_paths:[],frame_records:[],
      ...
      router_mode:w.router_mode||'auto',selected_provider:null,provider_metadata:{},provider_usage:{paid_generations:0,paid_repairs:0},approval_state:'pending',retention:{keep_final_spritesheet:true,keep_final_frames:false,keep_failed_frames:false,keep_contact_sheets:false,keep_preview_gifs:false,keep_experience_history:true,pinned_artifact_ids:[]},storage_state:'temporary',temporary_artifact_count:0,retained_artifact_count:0,cleanup_pending:false,
  ...
  function regenerate(w,runId){const old=get(w,runId);if(w.animation_runs.length>=50)throw Error('Maximum 50 runs');const r=copy(old);r.id=id();r.parent_run_id=old.id;r.created_at=now();invalidate(r);Object.assign(r,{status:'draft',approval_state:'pending',source_video_path:null,source_frame_paths:[],frame_records:[],selected_provider:null,provider_metadata:{},provider_usage:{paid_generations:0,paid_repairs:0},router_decision:null,technical_validation:null,semantic_validation:null,combined_validation:null,current_preview_id:null,current_contact_sheet_id:null,repair_plans:[],repair_history:[],semantic_reinspection_required:false,pipeline_progress:null});w.animation_runs.push(r);return copy(r);}
