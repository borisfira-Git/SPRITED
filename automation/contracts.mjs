...
const imageProvider={type:'string',enum:['external_manual','local_process','chatgpt_assisted']};
...
export const actions = {
  ...
  'agent/generate-animation':{tool:'generate_animation',description:'Create a new direct-frame animation request with an optional configured image provider.',properties:{character_id:text,animation_type:{type:'string',enum:['IDLE','HIT','DEATH','ATTACK','RANGE_ATTACK','WALKING']},provider:imageProvider},required:['character_id','animation_type']},
  ...
  'agent/submit-frame':{tool:'submit_frame',description:'Store one frame from manual bytes or the trusted configured local process. Commands cannot be supplied here.',properties:{animation_id:text,frame_index:number(0,23,true),provider:imageProvider,format:{type:'string',enum:['png','webp']},image_base64:imageBase64,instruction:text},required:['animation_id','frame_index']},
  'agent/replace-frame':{tool:'replace_frame',description:'Replace one stable frame using manual bytes or the trusted configured local process.',properties:{animation_id:text,frame_id:text,provider:imageProvider,format:{type:'string',enum:['png','webp']},image_base64:imageBase64,instruction:text},required:['animation_id','frame_id']},
  ...
  'agent/prepare-assisted-request':{tool:'prepare_assisted_request',description:'Prepare a ChatGPT-ready frame or repair prompt with stable SPRITED asset references; performs no browser or API action.',properties:{animation_id:text,frame_index:number(0,23,true),repair_plan_id:text,instruction:text},required:['animation_id','frame_index']},
  ...
  'agent/submit-repair-frame':{tool:'submit_repair_frame',description:'Replace only one planned frame using manual bytes or the trusted configured local process.',properties:{repair_plan_id:text,frame_index:number(0,23,true),provider:imageProvider,format:{type:'string',enum:['png','webp']},image_base64:imageBase64,instruction:text},required:['repair_plan_id','frame_index']},
