...
export const actions = {
  ...
  'agent/get-repair-plan':{tool:'get_repair_plan',description:'Create or retrieve a targeted repair plan from current validation results.',properties:{animation_id:text},required:['animation_id']},
  'agent/submit-repair-frame':{tool:'submit_repair_frame',description:'Replace one planned frame with externally supplied PNG/WebP content.',properties:{repair_plan_id:text,frame_index:number(0,23,true),format:{type:'string',enum:['png','webp']},image_base64:imageBase64},required:['repair_plan_id','frame_index','format','image_base64']},
  'agent/evaluate-repair':{tool:'evaluate_repair',description:'Rebuild preview and contact sheet, then rerun deterministic validation after repair.',properties:{repair_plan_id:text},required:['repair_plan_id']},
  'agent/get-animation-status':{tool:'get_animation_status',description:'Get compact validation, repair and export readiness for an animation.',properties:{animation_id:text},required:['animation_id']},
