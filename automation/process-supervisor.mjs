...
export function deprioritizeLowPerformingStrategies(ranking,disabled=[]){
  if(!disabled.length)return structuredClone(ranking);
...
export function observeRepairSession(run,{technical,semantic=null,strategyStats=[],promptComplexity=0,baseWasBestKnown=true,now=()=>new Date().toISOString()}={}){
  ...
  const limits=SUPERVISOR_THRESHOLDS[session.strictness],plan=(run.repair_plans||[]).at(-1)||{},beforeScore=score(plan.validator_scores_before?.overall)??score(plan.semantic_score)??score(plan.previous_validation_score)??session.best_known_result.score,currentScore=overall(technical,semantic),beforeTargetScore=score(plan.validator_scores_before?.target_issue)??score(plan.target_issue_score)??beforeScore,currentTargetScore=score(semantic?.target_issue_score)??score(technical?.target_issue_score)??currentScore,beforeIssues=unique(plan.issue_types_before||session.best_known_result.issue_types),afterIssues=unique([...issuesFrom(technical),...issuesFrom(semantic)]),persistent=beforeIssues.filter(item=>afterIssues.includes(item)),newIssues=afterIssues.filter(item=>!beforeIssues.includes(item)),strategies=unique((plan.frames_to_repair||[]).map(item=>item.strategy_id||item.repair_action)),phases=unique((plan.frames_to_repair||[]).map(item=>item.phase)),promptSize=promptComplexity||Math.max(0,...(plan.repair_results||[]).map(item=>item.instruction_summary?.length||0)),delta=currentScore===null||beforeScore===null?0:currentScore-beforeScore,targetDelta=currentTargetScore===null||beforeTargetScore===null?delta:currentTargetScore-beforeTargetScore,bestScore=session.best_known_result.score,regressive=currentScore!==null&&bestScore!==null&&currentScore<bestScore||newIssues.some(item=>critical.has(item));
  const observation={attempt:plan.attempt||session.observations.length+1,score:currentScore,before_score:beforeScore,target_issue_score:currentTargetScore,before_target_issue_score:beforeTargetScore,overall_delta:delta,target_issue_delta:targetDelta,issue_types:afterIssues,persistent_issues:persistent,new_issues:newIssues,affected_frames:unique([...framesFrom(technical),...framesFrom(semantic)]),affected_phases:phases,strategies,prompt_complexity:promptSize,regressive,base_was_best_known:baseWasBestKnown,created_at:now()};session.observations.push(observation);
  ...
  if(observations.filter(item=>item.issue_types.includes('incorrect_same_side_repetition')).length>=2)triggers.push('repeated_paired_frame_failure');
  if(observations.filter(item=>item.issue_types.includes('silhouette_jump')).length>=2)triggers.push('repeated_silhouette_over_preservation');
  ...
  let runningBest=score(run.process_supervisor?.best_known_result?.score);const bestTrend=observations.map(item=>{if(item.score!==null&&(runningBest===null||item.score>runningBest)&&!item.regressive)runningBest=item.score;return runningBest;});session.metrics={retry_count:observations.length,consecutive_non_improving_retries:nonImproving,consecutive_regressions:regressions,persistent_issue_count:persistent.length,distinct_strategies_tried:allStrategies.length,repeated_low_performing_strategy:selectedPoor||null,target_issue_score_trend:observations.map(item=>item.target_issue_score),best_known_result_trend:bestTrend,new_issues_introduced:unique(observations.flatMap(item=>item.new_issues)),prompt_complexity_trend:observations.map(item=>item.prompt_complexity)};
