...
try {
  if(options.api){
    ...
    service={close:async()=>{},readContentReference:async reference=>{const match=/^(character|frame|contact-sheet):([a-zA-Z0-9_-]+)$/.exec(reference);if(!match)throw Error('Unsupported asset reference');const response=await fetch(new URL(`/content/${match[1]}/${match[2]}`,base),{headers:{authorization:'Bearer '+token}});if(!response.ok){let message='Asset reference is unavailable';try{message=(await response.json()).errors?.[0]||message;}catch{}throw Error(message);}const mime_type=response.headers.get('content-type')?.split(';')[0];if(!['image/png','image/webp'].includes(mime_type))throw Error('Unsupported asset response');const bytes=Buffer.from(await response.arrayBuffer());if(!bytes.length||bytes.length>12*1024*1024)throw Error('Invalid asset response');return {bytes,mime_type};},call:async(action,args={})=>{const reads=['connections/status','connections/setup','status','frames/get','character/show','character/list','character/trash','recipes/list','providers/list','router/status','diagnostics/status','animation/list','animation/status','jobs/list','jobs/get','attempts/list'];let endpoint=action;
