...
export const MCP_INSTRUCTIONS='SPRITED is the local control plane for character animation. When the user uploads a character image and asks for an animation, first register or select a character profile, then call generate_animation with the user prompt and exact frame count. SPRITED uses its configured external Video Generator API, waits for the job, downloads the video, extracts frames, removes the configured solid background, aligns them, and returns PNG, JSON, GIF and ZIP artifacts. Use upload_video for an existing clip. Character profiles retain their generation history. Never request, transmit, or expose provider API keys; they exist only in the server environment.';
