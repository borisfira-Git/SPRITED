...
export class Service {
  ...
  async execute(action,args){
    ...
    try {
      ...
      if(action==='agent/semantic-validate-animation'){result=await this.semanticValidateAnimation(args);return {...envelope(result),status:result.validation.passed?'passed':'issues_found'};}
      if(action==='agent/get-repair-plan'){result=await this.getRepairPlan(args.animation_id);return {...envelope(result),status:result.overall_passed?'passed':'needs_repair'};}
      if(action==='agent/submit-repair-frame'){result=await this.submitRepairFrame(args);return {...envelope(result),status:'repair_submitted',next_suggested_action:'Submit remaining planned frames or call evaluate_repair.'};}
