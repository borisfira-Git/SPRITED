...
export function validateAnimation(input,{background='#00ff00',loop=true,pose_plan=null,motion_observations={}}={}){
  const issues=[],similarity_diagnostics=[],indexes=input.map(frame=>frame.frame_index),unique=new Set(indexes),sorted=[...input].sort((a,b)=>a.frame_index-b.frame_index),expected=Array.from({length:sorted.length},(_,i)=>i);
  ...
  return {animation_id:first?.animation_id||null,passed:issues.length===0,score,frame_count:input.length,issues,bad_frames,similarity_diagnostics,loop_validation:loopValidation,confidence:.8,confidence_label:'medium',phase_accuracy:null,phase_accuracy_confidence:.1,pose_plan_version:pose_plan?.version??null,motion_progression_diagnostics,paired_frame_diagnostics:[],metric_notes:'Deterministic image validity, bounds and ordering checks. Similarity is diagnostic only; motion approval comes from observed temporal validation.'};
}
